...
func TestBasicExprs(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			goq:  ID.NotIn([]int{1, 2, 3}),
			sql:  "`users`.`id` NOT IN ($1, $2, $3)",
			args: []interface{}{1, 2, 3},
		},
		{
			goq:  ID.In(z.Select(z.Var(1))),
			sql:  "`users`.`id` IN (SELECT $1)",
			args: []interface{}{1},
		},
		{
			goq:  z.Parens(ID.Add(4).Sbt(3)).Mlt(2).Dvd(1),
			sql:  "(`users`.`id` + $1 - $2) * $3 / $4",
			args: []interface{}{4, 3, 2, 1},
		},
		{
			goq:  Name.Concat("a").Concat(Name),
			sql:  "`users`.`name` || $1 || `users`.`name`",
			args: []interface{}{"a"},
		},
		{
			// An alias is ignored.
			goq:  z.Var(1).Add(z.Var(3).As("foo")),
			sql:  "$1 + $2",
			args: []interface{}{1, 3},
		},
		{
		...
		{
			goq:  z.Func("foo", 1, ID, 2).Add(3),
			sql:  "foo($1, `users`.`id`, $2) + $3",
			args: []interface{}{1, 2, 3},
		},
		{
			goq:  z.Coalesce(Name, z.Var(20)),
		...
		{
			goq:  z.Select(ID, Name, z.Var(1).Add(ID).As("test")),
			sql:  "SELECT `users`.`id`, `users`.`name`, $1 + `users`.`id` AS `test`",
			args: []interface{}{1},
		},
		{
			goq:  z.SelectDistinct(ID),
			sql:  "SELECT DISTINCT `users`.`id`",
			args: nil,
		},
		{
			// Allow to override SELECT.
			goq:  z.Select(Name).From(Users).Select(z.Count(ID)),
			sql:  "SELECT COUNT(`users`.`id`) FROM `users`",
			args: nil,
		},
		{
			goq: z.Select(Users.All()).From(Users).Joins(
		...
		{
			goq: z.Select(ID).From(Users).Where(
		...
		{
			goq:  z.Select(z.Select(z.Var(1)).As("subquery")),
			sql:  "SELECT (SELECT $1) AS `subquery`",
			args: []interface{}{1},
		},
		{
			goq: z.Select(z.Var(1)).From(
		...
		{
			goq:  z.Select(z.Var(1)).From(Users).Joins(Join(Users).On(Users.ID.Eq(3))),
			sql:  "SELECT $1 FROM `users` INNER JOIN `users` ON `users`.`id` = $2",
			args: []interface{}{1, 3},
		},
		{
			goq:  z.Select(z.Var(1).As("num")).OrderBy(z.Name("num")),
			sql:  "SELECT $1 AS `num` ORDER BY `num`",
			args: []interface{}{1},
		},
		{
			goq:  z.Select(z.Var(1)).OrderBy(Name.Asc(), ID.Desc(), ID),
			sql:  "SELECT $1 ORDER BY `users`.`name`, `users`.`id` DESC, `users`.`id`",
			args: []interface{}{1},
		},
		{
			// Postgres needs type information for placeholders in some cases.
			goq:  z.Select(z.VarT(1, "int")),
			sql:  "SELECT $1::int",
			args: []interface{}{1},
		},
		{
			goq:  z.Select(z.Null().As("a")).From(Users).Where(z.Null().Eq(z.Null())),
			sql:  "SELECT NULL AS `a` FROM `users` WHERE NULL = NULL",
			args: nil,
		},
		{
			goq:  z.Select(Users.Except(Users.ID), z.Null()).From(Users),
			sql:  "SELECT `users`.`name`, NULL FROM `users`",
			args: nil,
		},
		{
			goq: z.Select(z.Raw("selected")).From(Users).Where(
		...
		{
			goq: z.InsertInto(Users).ValuesMap(Values{
				Users.ID:   1,
				Users.Name: "bob",
			}),
		...
		{
			goq:  z.InsertInto(Users).Values(user{1, "bob"}),
			sql:  "INSERT INTO `users` VALUES ($1, $2)",
			args: []interface{}{1, "bob"},
		},
		{
			goq:  z.InsertInto(Users, Users.Name).Values(user{1, "bob"}),
		...
		{
			goq:  z.Update(Users).Elem(user{45, "john"}),
			sql:  "UPDATE `users` SET `id` = $1, `name` = $2 WHERE `users`.`id` = $3",
			args: []interface{}{45, "john", 45},
		},
		{
			goq:  z.Update(Users).Elem(user{45, "john"}, Users.Name),
			sql:  "UPDATE `users` SET `name` = $1 WHERE `users`.`id` = $2",
			args: []interface{}{"john", 45},
		},
		{
			goq:  z.DeleteFrom(Users),
